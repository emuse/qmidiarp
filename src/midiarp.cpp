...
bool MidiArp::isArp(snd_seq_event_t *evIn) {
  ...
  if ((evIn->data.control.channel < chIn) || (evIn->data.control.channel > chIn)) {
    return(false);
  }       
  if (((evIn->data.note.note < indexIn[0]) || (evIn->data.note.note > indexIn[1])) 
...
void MidiArp::getNote(snd_seq_tick_time_t *tick, int note[], int velocity[], int *length) { 
  ...
  *tick = arpTick + clip(tempo * 0.25 * (double)randomTick, 0, 1000, &outOfRange);
      ...
      if (c.isDigit() || (c == 'p')) {
        ...
        switch(c.toAscii()) { 
          ...
          case ')': chordMode = false;
            tmpIndex[chordIndex] = -1; // mark end of chord
            gotCC = false;
            break;
          case '+': octave++;
            break;
  ...
  do {
    ...
    note[l1] = clip(notes[noteBufPtr][0][noteIndex[l1]] + octave * 12, 0, 127, &outOfRange);
    grooveTmp = (grooveIndex % 2) ? -grooveVelocity : grooveVelocity;
    velocity[l1] = clip((double)notes[noteBufPtr][1][noteIndex[l1]] * vel * (1.0 + 0.005 * (double)(randomVelocity + grooveTmp)), 0, 127, &outOfRange);
    l1++;
  ...
  *length = clip(len * tempo * (double)TICKS_PER_QUARTER * (1.0 + 0.005 * (double)(randomLength + grooveTmp)), 2, 1000000,  &outOfRange);
  lastArpTick = arpTick;
  grooveTmp = (grooveIndex % 2) ? -grooveTick : grooveTick;
  arpTick += tempo * (double)TICKS_PER_QUARTER * (1.0 + 0.005 * (double)grooveTmp);
  if (!(patternLen && noteCount) || pause) {
    velocity[0] = 0;
  }  
  grooveIndex++;
}
...
void MidiArp::getNextNote(snd_seq_tick_time_t currentTick, snd_seq_tick_time_t *tick, int note[], int velocity[], int *length, bool *isNew) { 
...
void MidiArp::getCurrentNote(snd_seq_tick_time_t currentTick, snd_seq_tick_time_t *tick, int note[], int velocity[], int *length, bool *isNew) { 
...
void MidiArp::newRandomValues() {
  ...
  randomTick = (double)randomTickAmp * (0.5 - (double)random()/(double)RAND_MAX);
  randomVelocity = (double)randomVelocityAmp * (0.5 - (double)random()/(double)RAND_MAX);
  randomLength = (double)randomLengthAmp * (0.5 - (double)random()/(double)RAND_MAX);
}
...
void MidiArp::newGrooveValues(int p_grooveTick, int p_grooveVelocity, int p_grooveLength) {
